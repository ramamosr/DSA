...
Explanation 2:
 ...
 Answer is evident.
     */
    //points to note: each element can appear only once in a subset.
    ...
    public int[] solve(int[] A) {
        ...
        // Sort the array. The minimum value will be between the 2 adjacent elements of the array.
        // Max elements will be the farthest in the array between start and the end
        Arrays.sort(A);
        ...
        for(int i =0; i<A.length/2;i++){
            maxVal+= A[A.length-i-1] - A[i];
        }
        result[0] = (int)Math.floorMod(maxVal,mod);
        result[1] = (int)Math.floorMod(minVal,mod);
        return result;
    }
    public static void main(String[] args) {
        MaxAndMinMagic mm = new MaxAndMinMagic();
        mm.solve(new int[]{3, 11, -1, 5});
    }
}
