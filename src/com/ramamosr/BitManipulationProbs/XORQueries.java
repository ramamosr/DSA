...
Similarly for other queries.
    ...
    public int[][] solve(int[] A, int[][] B) {
        ...
        int[] pSumArr = new int[A.length];
        pSumArr[0] = A[0];
        for(int i = 1; i<A.length;i++){
            pSumArr[i] = pSumArr[i-1] + A[i];
        }
        for(int j =0; j<B.length;j++){
            int starIndex = B[j][0]-1;
            int endIndex = B[j][1]-1;
            if(starIndex > 0) {
                result[j][0] = (pSumArr[endIndex] - pSumArr[starIndex - 1])% 2;
                result[j][1] = (endIndex - starIndex +1) - (pSumArr[endIndex] - pSumArr[starIndex-1]);
            }
            else {
                result[j][0] = pSumArr[endIndex] % 2;
                result[j][1] = (endIndex - starIndex + 1) - pSumArr[endIndex];
            }
        }
