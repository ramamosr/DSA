package com.rammosr.DynamicProgramming;
...
dp[B] will be our answer. Remember to take modulo 1000007.
    ...
    public int coinchange2(int[] A, int B) {
        ...
        int[] noOfWays = new int[B+1];
        noOfWays[0] = 1;

        for(int i =0;i<A.length;i++)
            for(int j = A[i];j<=B;j++)
                noOfWays[j] += noOfWays[j-A[i]]%1000007;
        return (noOfWays[B]%1000007);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(cc.coinchange2(A,B));
    }
}
