...
There are N (N > 0) beggars sitting in a row outside a temple. Each beggar initially has an empty pot. When the devotees come to the temple,

 they donate some amount of coins to these beggars. Each devotee gives a fixed amount of coin(according to his faith and ability) to some K beggars sitting next to each other.

Given the amount donated by each devotee to the beggars ranging from i to j index, where 1 <= i <= j <= N, find out the
...
Now if we calculate the prefix sum at every index, we get [10, 55, 45, 25, 25].
    ...
    public int[] solve(int A, int[][] B) {
        ...
        result[0] = sum[0];
        for(int j=1;j<A;j++){
            result[j] = result[j-1] + sum[j];
        }
        return result;
    }

    public static void main(String[] args) {
        BeggarsOutSideTemple beg = new BeggarsOutSideTemple();
